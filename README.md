...
<u>**br_stat_profiler**</u> profiles the bias between machine base quality bias and empircial results. For that, it uses as an input, recalibration file generated by GATK BaseRecalibrator as a preprocessing step in Base Quality Sequence Recalibrator (BQSR) workfloe.  The GATK recalibration file provides a comprehensive summary of mismatch counts and observations for all identified combinations of quality scores and genomic context (k-mers). The tool then calculates for each k-mer the average QError (the difference between the base quality score and the empirical score). Wobbled k-mers statistics can be also calculated. The generated profile are uniform (same row number and composition) which enables downstream comparison and clustering.
...
3. **Degenerate bases inclusion:**&nbsp; Include wobbled k-mers and compute their QScore and empirical data from the non-wobbled k-mers set. The wobble statistics calculation is based on the non-wobbled k-mers. (optional, the user has control over the wobble compotion)
...
6. **Addition of a unique id:**  To enable downstream merging of multiple profiles, a unique ID for each row is added
